...
Covid.Bdry <- merge(x=Bdry,y=LA_clusters1,by.x="LAD20CD",by.y="area",all.x=FALSE)
Covid.Bdry.ll <- spTransform(Covid.Bdry,CRS("+proj=longlat +datum=WGS84 +no_defs"))
#cluster is the variable of interest we would like to be shown by colours
variable <- Covid.Bdry.ll@data$cluster
colours <- colorFactor(palette=c("#e41a1c","#377eb8"),domain=variable,reverse=FALSE)
...
leaflet(data=Covid.Bdry.ll) %>% addTiles() %>% addPolylines(opacity=1,weight=1,color="black") %>%
  ...
  addLegend(pal=colours,values=variable,opacity=1,title="Clusters") %>%
...
#total number of LADs at higher risk
nrow(LA_high)
#number of LADs in lower half (more deprived)
sum(LA_high$imd_rank<=156)
#number of LADs amongst 50 most deprived LADs in the English mainland
sum(LA_high$imd_rank<=50)
...
Covid.Bdry <- merge(x=Bdry,y=LA_clusters3,by.x="LAD20CD",by.y="area",all.x=FALSE)
Covid.Bdry.ll <- spTransform(Covid.Bdry,CRS("+proj=longlat +datum=WGS84 +no_defs"))
variable <- Covid.Bdry.ll@data$cluster
colours <- colorFactor(palette=c("#e41a1c","#377eb8"),domain=variable,reverse=FALSE)
...
#get the total number of LADs in the higher risk cluster
nrow(LA_high)
#how many LADs in the higher risk cluster are amongst the lower half of IMD ranking (more deprived)?
sum(LA_high$imd_rank<=156)
#how many LADs in higher risk cluster are amongst 50 most deprived LADs in English mainland?
sum(LA_high$imd_rank<=50)
